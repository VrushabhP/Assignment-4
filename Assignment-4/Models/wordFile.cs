...
namespace Office_File_Formats.Models
{
    class wordFile
    {
        ...
        {
            List<Student> studentlist = wordFile.readCSV(Constants.Locations.StudentCSVPath);
            using (WordprocessingDocument wordDocument = WordprocessingDocument.Create(filepath, WordprocessingDocumentType.Document))
            {
                ...
                foreach (var student in studentlist)
                {
                    if (student.StudentId.Length < 10)
                    {
                        ...
                        run.AppendChild(new Text($"StudentID Number '{ student.StudentId}'"));
                        run.AppendChild(new Break());
                        run.AppendChild(new Text($"Birthdate '{ student.DateOfBirth}'"));
                        run.AppendChild(new Break());
                        run.AppendChild(new Break());
        ...
        public static List<String> CSVFile(String FilePath)
        {
            String Contents;
        ...
        public static List<Student> readCSV(String FilePath)
        {
            ...
            List<String> entries = wordFile.CSVFile(FilePath);
            foreach (var a in entries)
            {
                Student tempstudent = new Student();
                tempstudent.FromCSV(a);
